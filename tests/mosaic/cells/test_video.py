...
class TestVideoCell(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        os.mkdir(TEMP_DIR)
        np.random.seed(42)
        video_arr = np.random.randint(0, 255,
                (MOCK_VIDEO_LENGTH, MOCK_VIDEO_SIZE, MOCK_VIDEO_SIZE, 3),
                dtype=np.uint8
                )
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        writer = cv2.VideoWriter(MOCK_VIDEO_FILE,
                fourcc,
                MOCK_VIDEO_FPS,
                (MOCK_VIDEO_SIZE, MOCK_VIDEO_SIZE)
            )
    ...
    def test_basic_construction(self):
        vid = self.get_video()
        self.assertEqual(vid.size(), (3, MOCK_VIDEO_LENGTH, MOCK_VIDEO_SIZE, MOCK_VIDEO_SIZE))
    ...
    def test_clip_lengths(self, clip_len):
        ...
        self.assertEqual(vid.size(), (DEFAULT_N_CLIPS, 3, clip_len, MOCK_VIDEO_SIZE, MOCK_VIDEO_SIZE))
        looped_vid = torch.cat([self.vid_tensor] * (clip_len // MOCK_VIDEO_LENGTH + 2), dim=1)
        for i, vid in enumerate(torch.split(vid, 1, dim=0)):
            start = i * MOCK_VIDEO_LENGTH // DEFAULT_N_CLIPS
            target_clip = torch.index_select(looped_vid, 1, torch.arange(start, start+clip_len))
    ...
    def test_more_clips_than_frames(self, n_clips):
        ...
        for i, vid in enumerate(torch.split(vid, 1, dim=0)):
            ...
            target_clip = torch.index_select(looped_vid, 1, torch.arange(start, start + DEFAULT_CLIP_LENGTH))
            self.assertTrue(torch.equal(vid.squeeze(0), target_clip))
    ...
    def test_composed_temporal_transforms(self, n_clips, clip_length, ratio):
        ...
        indices = torch.arange(0, self.vid_tensor.size(1), ratio)
        downsampled_clip = torch.index_select(self.vid_tensor, 1, indices) # contingent on previous test case
        looped_clip = torch.cat([downsampled_clip] * (clip_length // downsampled_clip.size(1) + 2), dim=1)
        for i, vid in enumerate(torch.split(vid, 1, dim=0)):
            start = i * downsampled_clip.size(1) // n_clips
            target_clip = torch.index_select(looped_clip, 1, torch.arange(start, start + clip_length))
            self.assertTrue(torch.equal(vid.squeeze(0), target_clip))
    ...
    def test_shape_without_stack_clips(self):
        transform_fn = Compose([
            TemporalDownsampling(DEFAULT_DOWNSAMPLE_RATIO, time_dim=1),
            TemporalCrop(DEFAULT_N_CLIPS, DEFAULT_CLIP_LENGTH, time_dim=1, stack_clips=False),
        ])
        vid = self.get_video(transform=transform_fn)
        self.assertEqual(vid.size(), (3, DEFAULT_N_CLIPS * DEFAULT_CLIP_LENGTH, MOCK_VIDEO_SIZE, MOCK_VIDEO_SIZE))
        indices = torch.arange(0, self.vid_tensor.size(1), DEFAULT_DOWNSAMPLE_RATIO)
        downsampled_clip = torch.index_select(self.vid_tensor, 1, indices) # contingent on previous test case
        looped_clip = torch.cat([downsampled_clip] * (DEFAULT_CLIP_LENGTH // downsampled_clip.size(1) + 2), dim=1)
        for i, vid in enumerate(torch.split(vid, DEFAULT_CLIP_LENGTH, dim=1)):
            start = i * downsampled_clip.size(1) // DEFAULT_N_CLIPS
            target_clip = torch.index_select(looped_clip, 1, torch.arange(start, start + DEFAULT_CLIP_LENGTH))
            self.assertTrue(torch.equal(vid, target_clip))
