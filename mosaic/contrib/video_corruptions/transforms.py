...
class TemporalDownsampling(object):
    """Video transformation for performing temporal downsampling (i.e. reading in
    every Nth frame only). This can be used in tandem with VideoCell by passing it
    into the `transform` keyword in the constructor. Can be used with Compose in
    torchvision.
    ...
    def __init__(self, downsample_factor: int, time_dim: Optional[int] = 1):
        ...
        if self.downsample_factor < 1:
            raise ValueError("Downsampling must be by a factor of 1 (no upsampling) or greater.")

    def __call__(self, video: torch.Tensor) -> torch.Tensor:
        video_length = video.size(self.time_dim)
        downsampled_indices = torch.arange(0, video_length, self.downsample_factor).long()
        frames = torch.index_select(video, self.time_dim, downsampled_indices)
        return frames


class TemporalCrop(object):
    """Video transformation for performing "temporal cropping:" the sampling of a
    pre-defined number of clips, each with pre-defined length, from a full video.
    Can be used with Compose in torchvision.
    ...
    Examples:
        ...
        cell = VideoCell("/path/to/video.mp4",
            time_dim=1,
            transform=TemporalCrop(8, 8, time_dim=1, clip_spacing="anywhere", padding_mode="freeze")
            )
    ...
    def __init__(self,
        ...
        if clip_length <= 0:
            raise ValueError("Clip length (# of frames per clip) must be a strictly positive integer.")
        if n_clips != int(n_clips):
            raise ValueError("Number of clips is not an integer")
        if n_clips <= 0:
            raise ValueError( "Number of clips must be a strictly positive integer")
