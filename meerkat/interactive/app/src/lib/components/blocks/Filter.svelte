<script lang="ts">
import type { FilterCriterion } from "$lib/api/datapanel";
    ...
    let handle_click = async (e) => {
        ...
        let return_value;
        if (checked) {
            return_value = await dispatch("filter", dp_box_id, filter_criteria);
            ...
            // dispatch({ type: 'filter', payload: { filter, dp_box_id, remove: true } });
        }
        // Dispatch unfilter when it's unchecked
        dispatch('filter', dp_box_id, filter);
    }
</script>
