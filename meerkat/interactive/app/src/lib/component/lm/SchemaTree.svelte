...
<button
	on:click={() => ($expand_all = !$expand_all)}
	class="bg-violet-200 rounded-sm p-1 font-mono text-sm text-violet-600"
	>Expand All {$expand_all}</button
>
<button
	on:click={() => ($hide_add_nodes = !$hide_add_nodes)}
	class="bg-violet-200 rounded-sm p-1 font-mono text-sm text-violet-600"
	>Hide Add Nodes {$hide_add_nodes}</button
>
<button
	on:click={() => delete_all_children('node1')}
	class="bg-violet-200 rounded-sm p-1 font-mono text-sm text-violet-600">Delete Selection</button
>
<button
	on:click={() => console.log(nodes)}
	class="bg-violet-200 rounded-sm p-1 font-mono text-sm text-violet-600">View Nodes</button
>
<Node node={nodes.node1} bind:nodes expanded={true} add_node={false} bind:this={root} />
